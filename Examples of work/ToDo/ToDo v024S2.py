...
# Functions
def AddTask():
    addT = [[sg.Text("Task Name:", font=font, justification='left'),
                      sg.Text("", size=11),
                      sg.Text("Due", font=font),
                      sg.Text("", size=4),
                      sg.Text("Status", font=font)],
                     [sg.InputText("", size=(15), key='-enterName-', font=font,background_color='#448ccf'),
                      sg.Input("", key='-timeAssign-', size=(10, 1), font=font, readonly=True,disabled_readonly_background_color='#2E75B6'),
                      sg.Combo(Add_Status, key="-StatusComb-", size=10, font=font, readonly=True,background_color='#448ccf')],
                     [sg.Text("Description:", font=font)],
                     [sg.MLine("", size=(23, 5), key='-enterDesc-', font=font,background_color='#448ccf'),
...
while True:
    ...
    if event == sg.WINDOW_CLOSED or event == '-Exodus-':
        ...
        if No_SelTask:
            sg.popup("SELECT A TASK.", keep_on_top=True, grab_anywhere=False, non_blocking=False)
        else:
            if table1:
                No_SelTask = True
                RowNum = values['-SystemOfATable-']
        ...
        if No_SelTask:
            sg.popup("SELECT A TASK.", keep_on_top=True, grab_anywhere=False, no_titlebar=True, non_blocking=False)
        else:
            if table1:
                RowNum = values['-SystemOfATable-']
                Rows = window['-SystemOfATable-'].get()
